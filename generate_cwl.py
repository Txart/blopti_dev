...
parser.add_argument('-d','--days', default=100, help='(int) Number of outermost iterations of the fipy solver, be it steadystate or transient. Default=10.', type=int)
...
sensors_canals_fn = Path(filenames_df[filenames_df.Content == 'sensors_canals'].Path.values[0])
...
sensors_canals, wtd_old , dem, peat_type_arr, peat_depth_arr = preprocess_data.read_preprocess_rasters(STUDY_AREA, wtd_old_fn, sensors_canals_fn, dem_rst_fn, peat_depth_rst_fn, peat_depth_rst_fn)
...
for i in range(0,N_ITER):
    ...
    wtd = hydro_standard.hydrology('transient', nx, ny, dx, dy, DAYS, ele, phi_ini, catchment_mask, wt_canal_arr, boundary_arr,
                                                      ...
                                                      P=P, ET=ET, dt=TIMESTEP, residue_difference=False, cwl_estimation=True, cwl_est_out_fn=cwl_est_out_fn,sensors_canals=sensors_canals)
